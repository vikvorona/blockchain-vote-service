...
const web3 = new web3API(
  new web3API.providers.HttpProvider("http://localhost:8008")
);

web3.eth.defaultAccount = web3.eth.accounts[0];

const pollAbi = web3.eth.contract(lib.poll.abi);

const accountAbi = web3.eth.contract(lib.account.abi);
// const web3 = null;
  ...
  router.get("/users", async (request, response) => {
    await passport.authenticate("jwt", (err, user) => {
      if (user && user.permissions === "admin") {
        ...
        response
          .status(500)
          .send({ error: "You don't have permissions for this action" });
      }
    })(request, response);
  });
    ...
    await passport.authenticate("jwt", (err, user) => {
      if (user && user.permissions === "admin") {
        if (!request.body.username || !request.body.password) {
          response
            .status(500)
            .send({ error: "Имя пользователя или пароль не заполнены." });
        } else {
          checkIfUserExists(request.body.username).then(
            ...
            (err) => {
              response
                .status(500)
                .send({ error: "Пользователь с таким именем существует" });
            }
          );
        }
      } else {
      ...
      contract.Voted((err, result) => {
        ...
        if (result.args.result === "success") {
          const index = answers.findIndex(
            (answer) => answer[0] === result.args.answer
          );
    ...
    await passport.authenticate("jwt", (err, user) => {
      if (user && user.permissions === "admin") {
        if (!request.body.name) {
          response
            .status(500)
            .send({ error: "Name and address of poll are required." });
        } else {
